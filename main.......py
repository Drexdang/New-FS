...
amounts_income_statement = [revenue, cost_of_goods_sold, operating_expenses, income_tax, other_income, other_expenses, operating_profit, net_profit]
...
if not any(np.isnan(amounts_income_statement)) and all(np.isfinite(amounts_income_statement)):
    labels = ['Revenue', 'COGS', 'Expenses', 'Income Tax', 'Other Income', 'Other Expenses', 'Operating Profit', 'Net Profit Or Loss']
    x = range(len(labels))
